        ...
        if rol == 'student':
            ...
            for user in grades:
                try:
                    ...
                    print(f"Notice: User '{user}' has registered ratings but does not exist in 'users' (key {e} is missing).")
            ...
            while change.lower() == "yes":
                ...
                if student in grades and subject in subjects:
                    ...
                    while new_grade is None:
                        try:
                            raw_grade = input("Grade New: ")
                            new_grade = float(raw_grade)

                            if not (0 <= new_grade <= 10):
                                raise ValueError("The rating must be between 0 and 10")
                        except ValueError as e:
                            if str(e).startswith("could not convert"):
                                print("Error: you must write a valid number (use decimal point, e.g. 9.5) Try again.")
                            else:
                                print(e)
                            new_grade = None
